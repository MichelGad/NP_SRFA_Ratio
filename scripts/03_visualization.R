...
for (measurement in unique_measurements) {
  ...
  vk_plot <- ggplot(plot_data, aes(x = formula_oc, y = formula_hc)) +
    ...
    labs(title = paste("Van Krevelen Diagram:", measurement), x = "O/C Ratio", y = "H/C Ratio",
...
iwa_plot <- ggplot(combined_data, aes(x = weighted_formula_hc, y = weighted_formula_oc, color = NP_type)) +
  ...
  labs(title = "Intensity-Weighted Averages: AuNP vs P25", subtitle = "Comparison of H/C and O/C Ratios",
...
# -----------------------------------------------------------------------------
message("Creating stacked column plots (common vs unique)...")
...
# -----------------------------------------------------------------------------
# Section: Van Krevelen variants driven by optional inputs
# -----------------------------------------------------------------------------
message("Creating Van Krevelen variants for clean and averaged data...")

# clean_bp_RI
vk_clean_bp_dir <- "van_krevelen/clean_bp_RI"
ensure_dir(file.path(output_base, vk_clean_bp_dir))
if (!is.null(formulas_clean) && all(c("measurement_name", "formula_oc", "formula_hc", "peak_relint_bp") %in% names(formulas_clean))) {
  for (measurement in unique(formulas_clean$measurement_name)) {
    ...
    vk_plot <- ggplot(plot_data, aes(x = formula_oc, y = formula_hc, color = peak_relint_bp)) +
      ...
      labs(title = paste("Van Krevelen Diagram:", measurement), x = "O/C Ratio", y = "H/C Ratio", caption = "RI(bp) is the peak intensity normalized to base peak intensity") +
      coord_cartesian(xlim = c(0, 1), ylim = c(0, 2)) +
      theme_bw(base_size = 14) +
      theme(plot.title = element_text(face = "bold", size = 18, hjust = 0.5), axis.title = element_text(face = "bold", size = 16), axis.text = element_text(size = 14), legend.position = "right", panel.grid.minor = element_blank(), plot.margin = margin(1, 0.5, 1, 1, "cm"), plot.caption = element_text(size = 10, color = "gray40", hjust = 0))
    filename <- paste0("vK_bp_", gsub("[^[:alnum:]]", "_", measurement), ".pdf")
    save_plot(vk_plot, file.path(vk_clean_bp_dir, filename))
  }
} else {
  message("  Missing inputs for clean_bp_RI (formulas_clean). Skipping.")
}

# avg_MFs_bp
vk_avg_bp_dir <- "van_krevelen/avg_MFs_bp"
ensure_dir(file.path(output_base, vk_avg_bp_dir))
if (!is.null(formulas_processed_all) && all(c("measurement_name", "formula_oc", "formula_hc", "peak_relint_bp") %in% names(formulas_processed_all))) {
  for (measurement in unique(formulas_processed_all$measurement_name)) {
    ...
    vk_plot <- ggplot(plot_data, aes(x = formula_oc, y = formula_hc, color = peak_relint_bp)) +
      ...
      coord_cartesian(xlim = c(0, 1), ylim = c(0, 2)) +
      theme_bw(base_size = 14) +
      theme(plot.title = element_text(face = "bold", size = 18, hjust = 0.5), axis.title = element_text(face = "bold", size = 16), axis.text = element_text(size = 14), legend.position = "right", panel.grid.minor = element_blank(), plot.margin = margin(1.5, 0.5, 2, 1, "cm"), plot.caption = element_text(size = 10, color = "gray40", hjust = 0.5))
    filename <- paste0("vK_bp_", gsub("[^[:alnum:]]", "_", measurement), ".pdf")
    save_plot(vk_plot, file.path(vk_avg_bp_dir, filename))
  }
} else {
  message("  Missing inputs for avg_MFs_bp (formulas_processed_all). Skipping.")
}

# avg_MFs_RI
vk_avg_ri_dir <- "van_krevelen/avg_MFs_RI"
ensure_dir(file.path(output_base, vk_avg_ri_dir))
if (!is.null(formulas_processed_all) && all(c("measurement_name", "formula_oc", "formula_hc", "rel_intensity_permille") %in% names(formulas_processed_all))) {
  for (measurement in unique(formulas_processed_all$measurement_name)) {
  ...
  message("  Missing inputs for avg_MFs_RI (formulas_processed_all). Skipping.")
}

# rep_MFs threshold coloring
...
if (!is.null(formulas_clean) && all(c("group", "occurrence_type", "peak_relint_bp") %in% names(formulas_clean))) {
  reproducibility_threshold_window <- formulas_clean %>%
    ...
    group_by(group) %>%
    summarise(threshold_min = quantile(peak_relint_bp, 0.025, na.rm = TRUE), threshold_max = quantile(peak_relint_bp, 0.975, na.rm = TRUE), .groups = "drop")
}

if (!is.null(formulas_processed_all) && "group" %in% names(formulas_processed_all) && !is.null(reproducibility_threshold_window)) {
  for (measurement in unique(formulas_processed_all$measurement_name)) {
    plot_data <- formulas_processed_all %>% filter(measurement_name == measurement) %>% arrange(rel_intensity_permille)
    if (nrow(plot_data) == 0) next
    ...
    plot_data <- plot_data %>%
      mutate(color_value = ifelse(rel_intensity_permille >= group_threshold$threshold_min & rel_intensity_permille <= group_threshold$threshold_max, NA_real_, rel_intensity_permille))
    values_vec <- scales::rescale(c(minv, group_threshold$threshold_min, group_threshold$threshold_max, maxv), to = c(0, 1))

    vk_plot <- ggplot() +
      geom_point(data = dplyr::filter(plot_data, is.na(color_value)), aes(x = formula_oc, y = formula_hc), color = "grey", alpha = 0.9, size = 3) +
      geom_point(data = dplyr::filter(plot_data, !is.na(color_value)), aes(x = formula_oc, y = formula_hc, color = color_value), alpha = 0.9, size = 3) +
  ...
  message("  Missing inputs for rep_MFs (formulas_processed_all/group/thresholds). Skipping.")
}
